...
# Shared session: connection pooling + transient retry, without changing the public sync API.
...
_SESSION.mount("https://", HTTPAdapter(max_retries=_RETRY, pool_connections=20, pool_maxsize=40))
_SESSION.mount("http://", HTTPAdapter(max_retries=_RETRY, pool_connections=10, pool_maxsize=20))
_GEO_CACHE = {}
_GEO_CACHE_T = {}
_GEO_TTL = 86400
_WEATHER_STALE_SEC = max(0, int(getattr(config, "WEATHER_STALE_SEC", 1800)))

def _request_get(url, *, params=None, headers=None, timeout=10):
...
def _coords(city: str):
    c = _norm_city(city)
    now = time.time() if 'time' in globals() else datetime.now().timestamp()
    if c in _GEO_CACHE and now - _GEO_CACHE_T.get(c, 0) < _GEO_TTL:
        return _GEO_CACHE[c] 
    if c in CITY_COORDS:
        return CITY_COORDS[c]
    now = time.time()
    if c in _GEO_CACHE and now - _GEO_CACHE_T.get(c, 0) < 86400:
        return _GEO_CACHE[c]
        ...
        if r.status_code == 200:
            for item in r.json().get("results") or []:
                ...
                if (item.get("country_code") or "").upper() in ("IR", "IQ", ""):
                    ...
                    _GEO_CACHE[c], _GEO_CACHE_T[c] = coords, now
                    return coords
...
def get_weather(city):
    ...
    if result:
        ...
        return result
    stale = _cache_data.get(city)
    if stale and now - _cache_time.get(city, 0) < ttl + _WEATHER_STALE_SEC:
        stale = dict(stale)
        stale["stale"] = True
        return stale
    return None
