...
def _price_action_analysis(opens, highs, lows, closes, vols=None, support=None, resistance=None, atr=None):
    """تحلیل پرایس‌اکشن چندعاملی؛ فقط از OHLCV همان تایم‌فریم استفاده می‌کند."""
    vols = vols or []
    n = len(closes)
    if n < 20:
    ...
    last_h = swing_highs[-1][1] if swing_highs else None
    prev_h = swing_highs[-2][1] if len(swing_highs) > 1 else None
    last_l = swing_lows[-1][1] if swing_lows else None
    prev_l = swing_lows[-2][1] if len(swing_lows) > 1 else None
    if last_h and prev_h and last_l and prev_l:
        if last_h > prev_h and last_l > prev_l: structure = "HH + HL | صعودی"
        elif last_h < prev_h and last_l < prev_l: structure = "LH + LL | نزولی"
        else: structure = "ساختار مختلط / رنج"
    else:
        structure = "ساختار نامشخص"

    # BOS / CHOCH نسبت به آخرین swing
    bos = "—"
    if last_h and closes[-1] > last_h: bos = "BOS صعودی"
    elif last_l and closes[-1] < last_l: bos = "BOS نزولی"
    elif prev_h and closes[-1] > prev_h and last_h and last_h < prev_h: bos = "CHOCH صعودی"
    elif prev_l and closes[-1] < prev_l and last_l and last_l > prev_l: bos = "CHOCH نزولی"

    i = n - 1
    b, r = body(i), rng(i)
    patterns = []
    if b/r < .25:
        patterns.append("Doji / عدم تصمیم")
    if lower(i) > b*2 and upper(i) < b and closes[i] >= opens[i]:
        patterns.append("Pin Bar صعودی")
    if upper(i) > b*2 and lower(i) < b and closes[i] <= opens[i]:
        patterns.append("Pin Bar نزولی")
    if n >= 2:
        pb = body(i-1)
        if closes[i] > opens[i] and closes[i] > opens[i-1] and opens[i] < closes[i-1] and b > pb:
            patterns.append("Bullish Engulfing")
        if closes[i] < opens[i] and closes[i] < opens[i-1] and opens[i] > closes[i-1] and b > pb:
            patterns.append("Bearish Engulfing")

    # شکست/رد سطح + close location
    location = "میانه"
    if resistance and closes[-1] > float(resistance):
        location = "بالای مقاومت"
    elif support and closes[-1] < float(support):
        location = "زیر حمایت"
    elif resistance and highs[-1] >= float(resistance):
        location = "تست/رد مقاومت"
    elif support and lows[-1] <= float(support):
        location = "تست/رد حمایت"

    # impulse / compression با ATR و حجم
    atr_v = float(atr or 0)
    impulse = "عادی"
    if atr_v and r >= 1.5 * atr_v: impulse = "Impulse قوی"
    elif atr_v and r <= .6 * atr_v: impulse = "Compression"

    vol_ratio = None
    if len(vols) >= 20:
        av = sum(float(x) for x in vols[-20:]) / 20
        vol_ratio = float(vols[-1]) / av if av else 1.0

    # liquidity sweep ساده: نفوذ به swing و برگشت داخل آن
    sweep = "—"
    if last_h and highs[-1] > last_h and closes[-1] < last_h: sweep = "Buy-side liquidity sweep"
    elif last_l and lows[-1] < last_l and closes[-1] > last_l: sweep = "Sell-side liquidity sweep"

    # نواحی supply/demand محلی بر اساس آخرین کندل مخالف قبل از impulse
    demand = supply = None
    look = min(n-1, 30)
    for j in range(n-2, max(1, n-look), -1):
        if closes[-1] > highs[j] and closes[j] < opens[j]:
            demand = (lows[j], max(opens[j], closes[j])); break
    for j in range(n-2, max(1, n-look), -1):
        if closes[-1] < lows[j] and closes[j] > opens[j]:
            supply = (min(opens[j], closes[j]), highs[j]); break

    return {
        "status": "ok", "structure": structure, "bos_choch": bos,
        "patterns": patterns[:4], "location": location, "impulse": impulse,
        "volume_ratio": round(vol_ratio, 2) if vol_ratio is not None else None,
        "liquidity_sweep": sweep, "last_swing_high": last_h, "last_swing_low": last_l,
