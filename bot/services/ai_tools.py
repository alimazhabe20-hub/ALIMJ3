...
def _register_builtin_tools() -> None:
    ...
    register_tool(
        ...
        description="موتور اصلی خرید داخل دستیار هوش مصنوعی. برای هر درخواست خرید، قیمت، فروشنده، لینک، مقایسه فروشگاه‌ها، ارزان‌ترین گزینه، شاپ اینستاگرام یا عکس محصول از این ابزار استفاده کن.  (ترب، دیجی‌کالا، اسنپ‌شاپ، تکنولایف، ایمالز، باسلام، مقدادآی‌تی، کالاوما، ۱۹کالا، موبایل‌آی‌آر، دیجی‌استایل، مدیسه، زنبیل، گلدیران، علی‌بابا، شیپور، دیوار، اکالا، تخفیفان) + صفحات فروش اینستاگرام + کل وب. وقتی کاربر قیمت/لینک خرید/شاپ اینستاگرام یا عکس محصول می‌خواهد از این ابزار استفاده کن. اگر مدل دقیق مشخص نیست با عبارت توصیفی جستجو کن. هرگز قیمت حدسی نگو.",
        parameters={
            "type": "object",
            "properties": {
